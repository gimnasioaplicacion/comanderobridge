...
if [ -d "$PODS_DIR" ]; then
  while IFS= read -r -d '' file; do
    perl -pi -e 's/SWIFT_VERSION\s*=\s*["\x27]?(?:3|3\.0)["\x27]?\s*;/SWIFT_VERSION = 5.0;/g; s/LastSwiftMigration\s*=\s*["\x27]?[^;"\x27]+["\x27]?\s*;/LastSwiftMigration = 1500;/g; s/LastUpgradeCheck\s*=\s*["\x27]?[^;"\x27]+["\x27]?\s*;/LastUpgradeCheck = 1500;/g' "$file"
